...
void main() {
  group('MccpProbe', () {
    ...
    test(
      'passes when server negotiates MCCP2 and sends compressed data',
      () async {
    ...
    test('fails when server sends WILL but no SB', () async {
      ...
      expect(
        result.message,
        contains('Server sent WILL MCCP2 but did not start compression'),
      );
    });
  });
}
