...
void main() {
  ...
  group('Self-Tests:', () {
    ...
    for (var file in fixturesDir.listSync().where(
      (e) => e.path.endsWith('.yaml'),
    )) {
      ...
      test('\${fixture.name} passes against GoodServer', () async {
        await TranscriptRunner.runSocket(
          fixture,
      ...
      test(
        '\${fixture.name} FAILS against BrokenIacServer (Mutation check)',
        () async {
          // Should throw TestFailure due to byte mismatch
          expect(
            TranscriptRunner.runSocket(
              fixture,
              InternetAddress.loopbackIPv4,
              brokenServer.port,
            ),
