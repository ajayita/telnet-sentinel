  ...
  Future<AuditResult> run(TelnetTransport transport) async {
    ...
    final subscription = transport.events.listen((event) {
      if (event.type == TelnetEventType.iac) {
        ...
        final currentState = stateManager.themStates[echoOption] ?? OptionState.no;
          ...
          if (currentState == OptionState.yes) {
            ...
            if (iterations >= maxIterations) {
              if (!completer.isCompleted) {
                completer.complete(AuditResult(name, AuditStatus.pass, 'Successfully toggled negotiation $maxIterations times without issues.'));
              }
            } else {
      ...
      if (!completer.isCompleted) {
        completer.complete(AuditResult(name, AuditStatus.fail, 'Connection error during negotiation loop: $error'));
      }
    }, onDone: () {
      if (!completer.isCompleted) {
        completer.complete(AuditResult(name, AuditStatus.fail, 'Connection closed during negotiation loop.'));
      }
    });
    ...
    try {
      ...
      return await completer.future.timeout(
        ...
        onTimeout: () {
          return AuditResult(name, AuditStatus.fail, 'Timeout waiting for negotiation loop to complete. Iterations: $iterations');
        },
      );
    } catch (e) {
      ...
      return AuditResult(name, AuditStatus.fail, 'Exception during Negotiation Loop probe: $e');
    } finally {
