  ...
  Future<AuditResult> run(TelnetTransport transport) async {
    ...
    final subscription = transport.events.listen((event) {
      ...
      if (!completer.isCompleted) {
        completer.complete(AuditResult(name, AuditStatus.fail, 'Error during GMCP probe: $error'));
      }
    });
    ...
    try {
      ...
      return await completer.future.timeout(
        ...
        onTimeout: () {
          ...
          if (willReceived) {
            reason = 'Server sent WILL GMCP but did not send any GMCP subnegotiation messages.';
          }
      ...
      return AuditResult(name, AuditStatus.fail, 'Exception during GMCP probe: $e');
    } finally {
